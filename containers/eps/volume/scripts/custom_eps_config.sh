...
check_vars(){
  ...
  if [ -z "${BITCOIN_PASSWORD}" ]; then printf "ENV VAR BITCOIN_PASSWORD not present\n" 1>&2; return 1; fi
  if [ -z "${BITCOIN_HOSTNAME}" ]; then printf "ENV VAR BITCOIN_HOSTNAME not present\n" 1>&2; return 1; fi
  if [ -z "${BITCOIN_PORT}" ]; then printf "ENV VAR BITCOIN_PORT not present\n" 1>&2; return 1; fi
  if [ -z "${BITCOIN_WALLET}" ]; then printf "ENV VAR BITCOIN_WALLET not present\n" 1>&2; return 1; fi
}
write_file(){
...
wallet_filename = ${BITCOIN_WALLET} 
